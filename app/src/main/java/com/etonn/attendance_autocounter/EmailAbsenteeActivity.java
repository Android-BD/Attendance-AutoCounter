...
public class EmailAbsenteeActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(Student absentee : absenteesList){
            tos[i] = absentee.getName().replace(" ", "_")+"@student.uml.edu";
            i++;
        }

        intent.setType("message/rfc822");
        intent.putExtra(Intent.EXTRA_EMAIL, tos);
        intent.putExtra(Intent.EXTRA_SUBJECT,"You were absent for Android course");
        intent.putExtra(Intent.EXTRA_TEXT,
                "Dear student," +
                        "\n\nYou were absent for last android class, be careful." +
